...
benign_data = read_csv(os.path.join(
    path, 'benign_data.csv'), header=None)
print("Load malware dataset.")
malware_data = read_csv(os.path.join(
    path, 'malware_data.csv'), header=None)

benign_train_data, benign_test_data = train_test_split(
    benign_data, test_size=0.2)
malware_train_data, malware_test_data = train_test_split(
    malware_data, test_size=0.2)
...
y = numpy.concatenate([numpy.zeros(benign_train_data.shape[0]),
                      numpy.ones(malware_train_data.shape[0])])
y_test = numpy.concatenate(
    [numpy.zeros(benign_test_data.shape[0]), numpy.ones(malware_test_data.shape[0])])
...
stacking_single = StackingClassifier(
    estimators=single_base, final_estimator=DecisionTreeClassifier(), n_jobs=None, cv=10)
stacking_ensemble = StackingClassifier(
    estimators=ensemble_base, final_estimator=RandomForestClassifier(), n_jobs=None, cv=10)
...
with open(str(results_file), "a+") as f_out:
    ...
    for clf in tqdm(stacking_list, desc="Progess: "):
        print(clf.__class__.__name__[0:8] +
              clf.final_estimator.__class__.__name__)
        result = list()
        clf.fit(x, y)
        ...
        result.extend(('\n' + clf.__class__.__name__[
                      0:8] + clf.final_estimator.__class__.__name__, auc, acc, precision, recall, f1))
        result = ",".join(result)
        f_out.write(result)
        disp = plot_confusion_matrix(clf, x_test, y_test, normalize='true', display_labels=[
                                     'Benign', 'Malware'], values_format='.3g', cmap="Blues")
        disp.plot()
        disp.figure_.savefig('CSV/confusion_' + clf.__class__.__name__[
                             0:8] + clf.final_estimator.__class__.__name__[:-10] + '.png')
        disp.figure_.clf()

        fpr, tpr, _ = roc_curve(y_test, y_prob)
        auc_list.append([fpr, tpr, clf.__class__.__name__[
                        0:8] + clf.final_estimator.__class__.__name__[:-10] + ", AUC="+str(auc)])
